...
void create_room(struct Room *room, char*  name,char* description, int position, int travelable_rooms[])
{
	...
	int blank_travelable_rooms[MAX_TRAVELABLE_ROOMS];
	int jeep;

	for( jeep  = 0; jeep<MAX_TRAVELABLE_ROOMS; jeep = jeep + 1){
		blank_travelable_rooms[jeep]=999;
	}

	for( jeep  = 0; jeep<MAX_TRAVELABLE_ROOMS; jeep = jeep + 1){
		room->travelable_rooms[jeep]=blank_travelable_rooms[jeep];
	}
	for( jeep  = 0; jeep<MAX_TRAVELABLE_ROOMS; jeep = jeep + 1){
...
void main(void){
	...
	int basement_connections[MAX_TRAVELABLE_ROOMS]; basement_connections[0] = 0; basement_connections[1] = 0; basement_connections[2] = 0;
	int living_room_connections[MAX_TRAVELABLE_ROOMS]; living_room_connections[0] = 0; living_room_connections[1] = 0; living_room_connections[2] = 0;
...
/// int base_cost,  int damage,  int defense);
	...
	create_noun(&nouns[3], "katana", 90, 3, 0, 0, "m", "he", "him", "fine sword", 100,  5,  1);
	create_noun(&nouns[4], "kimono", 90, 4, 0, 0, "m", "he", "him", "traditional silk dress", 100,  5,  1);
...
/// create_room(struct Room *room, char*  name,char* description, int position, int travelable_rooms[MAX_TRAVELABLE_ROOMS]);
	...
	create_room(&rooms[1], "room", "the living room, there are walls of anime figurine displays . stairs lead down to the basement ", 1, living_room_connections );
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 //static const char *nouns[] = { " candy", " meowni", " shirt", " skirt", "", "", "", "", "", ""};
	...
	void parse(struct Noun * nouns, struct Room * rooms,int gamerun,int time){
		...
		verb_parse = 0; /// NULL POINTER
		direct_object_parse = 0;
		room_noun_parse = 0;
		...
		static const char *dictionary_full[] = { " pat", " tickle", " kill", " dev", " examine", " go", " take", " drop", " wield", " unwield", " attack", "to", "anon", "loli", "shota", "katana",
		"kimono", "basement", "living room", "BUFFER NAME"};
...
if (preposition_parse == 0){
					...
					for( indirect_object = 0; (strcmp(nouns[indirect_object].name, "BUFFER NAME") && indirect_object_parse); indirect_object = indirect_object + 1 ){
						...
						for( x ; ((x < tokencounter) && indirect_object_parse) ; x++ ){
//							 printf("indirect_object_parse = %d \n nouns[indirect_object].name = .%s. \n tokenized_parse[%d] = .%s. \n",indirect_object_parse, nouns[indirect_object].name,x,tokenized_parse[x]);
		 	   indirect_object_parse = strcmp(nouns[indirect_object].name, tokenized_parse[x]);
			...
			}indirect_object--; ///for indirect_object end
		 	...
		 	direct_object_parse = strstr(string_to_parse, nouns[direct_object].name);
	...
	//		printf("verb parse: %s", verb_parse);
